...
class RumahFilm_get:
	...
	def j_popular(request):
		...
		response_data["popular"] = RumahFilm_get.get_film_dat("popular",0)
		return HttpResponse(json.dumps(response_data),content_type="application/json")
	def j_recomended(request):
		response_data = {}
		response_data["trend"] = RumahFilm_get.get_film_dat("t",0)['results']
		return HttpResponse(json.dumps(response_data),content_type="application/json")
	def j_search(request,data):
		response_data = {}
		response_data["trend"] = RumahFilm_get.get_film_dat("search",0)['results']
		return HttpResponse(json.dumps(response_data),content_type="application/json")
	def j_top(request):
		response_data = {}
		response_data["trend"] = RumahFilm_get.get_film_dat("t",0)['results']
	...
	def get_details(request,id):
		response_data = {}
		# query = "https://api.themoviedb.org/3/movie/popular?api_key="+API_key+"&language=en-US&page=500"
		response_data["detail"] = RumahFilm_get.get_film_dat("d_f",id)['title']
